...
void EnemySlime::DrawDebugPrimitive()
{
    ...
    //縄張り範囲をデバッグ円柱描画
    debugRenderer->DrawCylinder(territoryOrigin, territoryRange,
    ...
    //索敵範囲をデバッグ円柱描画
    debugRenderer->DrawCylinder(position, searchRange, 
        1.0f, DirectX::XMFLOAT4(0, 0, 1, 1));

    //攻撃範囲をデバッグ円柱描画
    debugRenderer->DrawCylinder(position, attackRange,
        1.0f, DirectX::XMFLOAT4(1, 0, 0, 1));
}
...
{
    //現在の位置からRangeの距離中でランダム位置を返す
    DirectX::XMFLOAT3 randomRangePos = position;

    targetPosition.x = Mathf::RandomRange(randomRangePos.x - territoryRange, 
        randomRangePos.x + territoryRange);
    targetPosition.y = Mathf::RandomRange(randomRangePos.y - territoryRange, 
        randomRangePos.y + territoryRange);
    targetPosition.z = Mathf::RandomRange(randomRangePos.z - territoryRange, 
        randomRangePos.z + territoryRange);
}
