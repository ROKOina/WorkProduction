...
void ModelResource::BuildModel(ID3D11Device* device, const char* dirname)
{
	for (Material& material : materials_)
	{
		...
		//HRESULT hr = DirectX::CreateWICTextureFromFile(device, wfilename, resource.GetAddressOf(), material.diffuseMap.GetAddressOf());
		//if (FAILED(hr))
		//{
		...
		//		hr = device->CreateShaderResourceView(texture.Get(), nullptr, material.diffuseMap.GetAddressOf());
		//		_ASSERT_EXPR(SUCCEEDED(hr), HRTrace(hr));
