#include "Misc.h"
#include "SilhoutteShader.h"
#include "Graphics\Graphics.h"

SilhoutteShader::SilhoutteShader(ID3D11Device* device)
{
	...
	// 頂点シェーダー
	{
		...
		dx11State->createVsFromCso(device, "Shader\\LambertVS.cso", vertexShader_.GetAddressOf(),
			inputLayout_.GetAddressOf(), inputElementDesc, ARRAYSIZE(inputElementDesc));
	}

	// ピクセルシェーダー
	{
		dx11State->createPsFromCso(device, "Shader\\SilhouttePS.cso", pixelShader_.GetAddressOf());
	}
...
// 描画開始
void SilhoutteShader::Begin(ID3D11DeviceContext* dc, const ShaderParameter3D& rc)
{
	...
	dc->OMSetDepthStencilState(
		dx11State->GetDepthStencilState(Dx11StateLib::DEPTHSTENCIL_STATE_TYPE::SILHOTTE).Get(),
		0);
	dc->RSSetState(
	...
	DirectX::XMStoreFloat4x4(&cbScene_.viewProjection, V * P);

	cbScene_.lightDirection = rc.lightDirection;
	dc->UpdateSubresource(sceneConstantBuffer_.Get(), 0, 0, &cbScene_, 0, 0);
}

// 描画
void SilhoutteShader::Draw(ID3D11DeviceContext* dc, const Model* model)
{
...
// 描画終了
void SilhoutteShader::End(ID3D11DeviceContext* dc)
{
